...
type SplitedName = { title: string, name: string, last_name: string }

export function splitPeopleName(fullName: string): SplitedName {
...
function seperateLastName({ title, name }): SplitedName {
  const splited = _.compact<string>(name.split(" "))
  return {
    title,
    name: splited[0],
